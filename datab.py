...
async def update_channel_db(ponto, ab, ba, notify):
    query = "UPDATE countTable SET ab = %s, ba = %s WHERE ponto = %s"
    await query_db(query, (ab, ba, ponto), commit=True)
    print("adicionado: ", ponto, ab, ba)
    query = "INSERT INTO countTable_dates (ponto, ab, ba, notify) VALUES (%s,%s,%s,%s)"
    await query_db(query, (ponto, ab, ba, notify), commit=True)
